...
export const updateMatchName = async (matchId: string, name: string) => {
    const response = await api.put(`/matches/${matchId}/name`, { name });
    return response.data;
};

// Update match dates
export const updateMatchDates = async (matchId: string, startDate: string, endDate: string) => {
    const response = await api.put(`/matches/${matchId}/dates`, { startDate, endDate });
    return response.data;
};
...
export const updateEvent = async (matchId: string, eventId: string, event: { name: string; description?: string }) => {
    const response = await api.put(`/matches/${matchId}/events/${eventId}`, event);
    return response.data;
};
...
export const callEvent = async (matchId: string, eventId: string) => {
    const response = await api.post(`/matches/${matchId}/events/${eventId}/call`);
    return response.data;
};

// Recall event (remove number)
export const recallEvent = async (matchId: string, eventId: string) => {
    const response = await api.post(`/matches/${matchId}/events/${eventId}/recall`);
    return response.data;
};
...
export const inviteUser = async (matchId: string, email: string) => {
    const response = await api.post(`/matches/${matchId}/invites`, { email });
    return response.data;
};

// Remove participant
export const removeParticipant = async (matchId: string, userId: string) => {
    const response = await api.delete(`/matches/${matchId}/participants/${userId}`);
    return response.data;
};

// Regenerate card for a user
export const regenerateUserCard = async (matchId: string, userId: string) => {
    const response = await api.post(`/matches/${matchId}/participants/${userId}/regenerate-card`);
    return response.data;
};

// Get match invites
export const getMatchInvites = async (matchId: string) => {
    const response = await api.get(`/matches/${matchId}/invites`);
    return response.data;
};

// Delete invite
export const deleteInvite = async (matchId: string, inviteId: string) => {
    const response = await api.delete(`/matches/${matchId}/invites/${inviteId}`);
    return response.data;
};
...
export const updateInvite = async (
    ...
    const response = await api.put(
        `/matches/${matchId}/invites/${inviteId}`,
        data
    );
    return response.data;
};