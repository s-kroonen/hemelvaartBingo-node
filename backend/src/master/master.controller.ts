...
export class MasterController {
    ...
    async participantRegenerateCard(@Param('matchId') matchId: string, @Param('userId') userId: string) {
        const match = await this.matchService.findById(matchId);
        if (!match) throw new NotFoundException(`Match with id ${matchId} not found`);
    ...
    async deleteParticipant(@Param('matchId') matchId: string, @Param('userId') userId: string) {
        const match = await this.matchService.findById(matchId);
        if (!match) throw new NotFoundException(`Match with id ${matchId} not found`);
        const user = await this.userService.findById(userId);
        if (!user) throw new NotFoundException(`User with id ${userId} not found`);
