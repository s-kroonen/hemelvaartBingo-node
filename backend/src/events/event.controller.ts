...
@UseGuards(FirebaseAuthGuard) // Entire controller is protected
export class EventController {
  ...
  async getLatest(@Param('matchId') matchId: string) {
    // Only returns the single most recent "called" event
    return this.eventService.getLatestCalled(matchId);
  }

  @Get('history')
  async getHistory(@Param('matchId') matchId: string) {
    // Returns all "called" events, newest to oldest
    return this.eventService.getCalledHistory(matchId);
  }
}
